...
def parse_config_into_pdc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Paydacoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/paydacoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/paydacoin_config.h'), "w", encoding="utf8") as pdc_config:
        pdc_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Paydacoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_pdc_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/paydacoin_config.h'), os.path.join(SOURCE_DIR, 'config/paydacoin-config.h'))
