#!/usr/bin/env python3
# Copyright (c) 2021 The Paydacoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

""" Interactive paydacoind P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for Paydacoin Core can enable. It uses BCC
...
def main(paydacoind_path):
    peers = dict()

    paydacoind_with_usdts = USDT(path=str(paydacoind_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    paydacoind_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    paydacoind_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[paydacoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/paydacoind")
        exit()
    path = sys.argv[1]
    main(path)
