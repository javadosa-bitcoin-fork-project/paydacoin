...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing paydacoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='paydacoin-cli', help='paydacoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Paydacoin address to which the faucet should send')
...
parser.add_argument('paydacoin_cli_args', nargs='*', help='Arguments to pass on to paydacoin-cli (default: -signet)')

args = parser.parse_args()

if args.paydacoin_cli_args == []:
    args.paydacoin_cli_args = ['-signet']


def paydacoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.paydacoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = paydacoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = paydacoin_cli(['getnewaddress', 'faucet', 'bech32'])
