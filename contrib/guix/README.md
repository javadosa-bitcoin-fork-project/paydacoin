# Bootstrappable Paydacoin Core Builds

This directory contains the files necessary to perform bootstrappable Paydacoin
Core builds.
...
To build Paydacoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Paydacoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/paydacoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/paydacoin-core/guix.sigs).
