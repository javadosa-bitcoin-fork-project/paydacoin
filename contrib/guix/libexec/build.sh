#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Paydacoin Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} PAYDACOIN_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Paydacoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Paydacoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example paydacoin.conf file. if contrib/devtools/gen-paydacoin-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/paydacoin.conf" "${DISTNAME}/"
