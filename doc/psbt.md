# PSBT Howto for Paydacoin Core

Since Paydacoin Core 0.17, an RPC interface exists for Partially Signed Paydacoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/paydacoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Paydacoin transactions that are not fully signed
...
#### Multisig with multiple Paydacoin Core instances
...
Paydacoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `paydacoin-cli` is used.
...
Later, when *V* PDC has been received on *Amulti*, and Bob and Carol want to
