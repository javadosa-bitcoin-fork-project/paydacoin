# I2P support in Paydacoin Core

It is possible to run Paydacoin Core as an
...
## Run Paydacoin Core with an I2P router (proxy)
...
Once an I2P router with SAM enabled is up and running, use the following Paydacoin
...
```
paydacoind -i2psam=127.0.0.1:7656
```
...
debug log about your I2P configuration and connections. Run `paydacoin-cli help
...
I2P support was added to Paydacoin Core in version 22.0 and there may be fewer I2P
peers than Tor or IP ones. Therefore, using I2P alone without other networks may
make a node more susceptible to [Sybil
attacks](https://en.paydacoin.it/wiki/Weaknesses#Sybil_attack). You can use
`paydacoin-cli -addrinfo` to see the number of I2P addresses known to your node.
...
The first time Paydacoin Core connects to the I2P router, it automatically
...
`i2p_private_key` in the Paydacoin Core data directory.  The persistent I2P
...
## Fetching I2P-related information from Paydacoin Core

There are several ways to see your I2P address in Paydacoin Core if accepting
incoming I2P connections (`-i2pacceptincoming`):
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`
- in the debug log (grep for `AddLocal`; the I2P address ends in `.b32.i2p`)

To see which I2P peers your node is connected to, use `paydacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `paydacoin-cli getpeerinfo`).
...
Paydacoin Core uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and Paydacoin Core

Paydacoin Core uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3)
...
To allow future upgrades to newer versions of SAM, Paydacoin Core sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of Paydacoin Core.
...
It's important that the nodes of a popular application like Paydacoin contribute
...
## Bundling I2P in a Paydacoin application

Please see the "General Guidance for Developers" section in https://geti2p.net/en/docs/api/samv3
if you are developing a downstream application that may be bundling I2P with Paydacoin.
