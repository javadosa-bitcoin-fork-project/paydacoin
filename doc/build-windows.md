...
Below are some notes on how to build Paydacoin Core for Windows.

The options known to work for building Paydacoin Core on Windows are:
...
Note that for WSL the Paydacoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/paydacoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\paydacoin`, for example:

    make install DESTDIR=/mnt/c/workspace/paydacoin
