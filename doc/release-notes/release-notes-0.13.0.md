Paydacoin Core version 0.13.0 is now available from:

  <https://paydacoin.org/bin/paydacoin-core-0.13.0/>
...
updates will be released anymore. Without security updates, using a paydacoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Paydacoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/paydacoin/paydacoin/issues/7681#issuecomment-217439891)
...
- Adding `dbcache=100` in paydacoin.conf
...
paydacoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/paydacoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Paydacoin Core code base has
...
- `paydacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `paydacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
compatible peers.
[BIP 152](https://github.com/paydacoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Paydacoin Core.

[Pull request](https://github.com/paydacoin/paydacoin/pull/8035/files), [BIP 32](https://github.com/paydacoin/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/paydacoin/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/paydacoin/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/paydacoin/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/paydacoin/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Paydacoin Core is expected that
...
Witness)] (https://github.com/paydacoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/paydacoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Paydacoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/paydacoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/paydacoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/paydacoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/paydacoin/paydacoin/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for paydacoin-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use PDC/kB (MarcoFalke)
...
- #7692 `29b2be6` Remove P2P alert system (pdcdrak)
...
- #8275 `042c323` Remove bad chain alert partition check (pdcdrak)
...
- #7619 `18b3f1b` Add missing sudo entry in gitian VM setup (pdcdrak)
- #7616 `639ec58`  [depends] Delete unused patches  (MarcoFalke)
- #7658 `c15eb28` Add curl to Gitian setup instructions (pdcdrak)
...
- #7613 `3798e5d` Add autocomplete to paydacoin-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add paydacoin address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: paydacoin-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from paydacoind (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Paydacoin-Core repository and website to the About Dialog (MarcoFalke)
...
- #7531 `18cb2d5` Add bip68-sequence.py to extended rpc tests (pdcdrak)
...
- #7773 `7c80e72` Fix comments in tests (pdcdrak)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for paydacoind procs (MarcoFalke)
...
- #8164 `0f24eaf` [Paydacoin-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of paydacoind startup (laanwj)
...
- #7975 `fc23fee` Update paydacoin-core GitHub links (MarcoFalke)
...
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (pdcdrak)
