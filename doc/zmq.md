...
The Paydacoin Core daemon can be configured to act as a trusted "border
router", implementing the paydacoin wire protocol and relay, making
...
The ZeroMQ feature in Paydacoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building paydacoind:
...
For instance:
    ...
    $ paydacoind -zmqpubhashtx=tcp://127.0.0.1:27332 \
               -zmqpubhashtx=tcp://192.168.1.2:27332 \
               -zmqpubhashblock="tcp://[::1]:27333" \
               -zmqpubrawtx=ipc:///tmp/paydacoind.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in paydacoin.conf.
...
From the perspective of paydacoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into paydacoind directly. Furthermore, no information is
...
using. Paydacoind appends an up-counting sequence number to each
