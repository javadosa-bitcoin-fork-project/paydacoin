# User-space, Statically Defined Tracing (USDT) for Paydacoin Core

Paydacoin Core includes statically defined tracepoints to allow for more
...
**Note**: Paydacoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Paydacoin Core
...
Multiple tools can list the available tracepoints in a `paydacoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Paydacoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/paydacoind
stap net        outbound_message 0x0000000000107c05 /src/paydacoind
stap validation block_connected  0x00000000002fb10c /src/paydacoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Paydacoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/paydacoind | grep NT_STAPSDT -A 4 -B 2
