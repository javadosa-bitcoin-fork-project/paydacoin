# CJDNS support in Paydacoin Core

It is possible to run Paydacoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Paydacoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Paydacoin network and individual nodes.
...
before it will work with your Paydacoin Core node. This is described in steps
...
## Run Paydacoin Core with CJDNS

Once you are connected to the CJDNS network, the following Paydacoin Core
...
When enabled, this option tells Paydacoin Core that it is running in an
...
IPv6 local network. This helps Paydacoin Core perform better address management:
...
CJDNS support was added to Paydacoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `paydacoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/paydacoin/paydacoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Paydacoin Core

There are several ways to see your CJDNS address in Paydacoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `paydacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `paydacoin-cli getpeerinfo`).
