...
const char * const PAYDACOIN_CONF_FILENAME = "paydacoin.conf";
const char * const PAYDACOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //paydacoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", PAYDACOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
#else
    const char* pszModule = "paydacoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Paydacoin
    // macOS: ~/Library/Application Support/Paydacoin
    // Unix-like: ~/.paydacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Paydacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Paydacoin";
#else
    // Unix-like
    return pathRet / ".paydacoin";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", PAYDACOIN_CONF_FILENAME));
}
