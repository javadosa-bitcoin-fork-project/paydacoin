...
/** Call node.updateRwSetting() with Paydacoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Paydacoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/paydacoin/paydacoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because paydacoin 22.x will not try to read these.
...
//! Convert enabled/size values to paydacoin -prune setting.
...
//! Get pruning enabled value to show in GUI from paydacoin -prune setting.
...
//! Get pruning size value to show in GUI from paydacoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayPaydacoinUnit")) {
        settings.setValue("DisplayPaydacoinUnit", QVariant::fromValue(PaydacoinUnit::PDC));
    }
    QVariant unit = settings.value("DisplayPaydacoinUnit");
    if (unit.canConvert<PaydacoinUnit>()) {
        m_display_paydacoin_unit = unit.value<PaydacoinUnit>();
    } else {
        m_display_paydacoin_unit = PaydacoinUnit::PDC;
        settings.setValue("DisplayPaydacoinUnit", QVariant::fromValue(m_display_paydacoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if paydacoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_paydacoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<PaydacoinUnit>() == m_display_paydacoin_unit) return;
    m_display_paydacoin_unit = new_unit.value<PaydacoinUnit>();
    QSettings settings;
    settings.setValue("DisplayPaydacoinUnit", QVariant::fromValue(m_display_paydacoin_unit));
    Q_EMIT displayUnitChanged(m_display_paydacoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/paydacoin/paydacoin/pull/8273
    ...
    // (https://github.com/paydacoin-core/gui/issues/567).
