...
#include <qt/paydacoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Paydacoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new PaydacoinAddressEntryValidator(parent));
    widget->setCheckValidator(new PaydacoinAddressCheckValidator(parent));
}
...
bool parsePaydacoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no paydacoin: URI
    if(!uri.isValid() || uri.scheme() != QString("paydacoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!PaydacoinUnits::parse(PaydacoinUnit::PDC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parsePaydacoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsePaydacoinURI(uriInstance, out);
}

QString formatPaydacoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("paydacoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PaydacoinUnits::format(PaydacoinUnit::PDC, info.amount, false, PaydacoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openPaydacoinConf()
{
    ...
    /* Open paydacoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Paydacoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Paydacoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Paydacoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Paydacoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "paydacoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("paydacoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a paydacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Paydacoin\n";
        else
            optionFile << strprintf("Name=Paydacoin (%s)\n", chain);
