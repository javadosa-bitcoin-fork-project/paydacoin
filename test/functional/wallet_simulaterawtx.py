...
class SimulateTxTest(PaydacoinTestFramework):
    ...
    def run_test(self):
        ...
        paydacoin_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 pdc decrease, w2 sees + 5 pdc
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + paydacoin_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - paydacoin_fee + 4.9999       -5 - paydacoin_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - paydacoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - paydacoin_fee)
        ...
        tx2 = funding["hex"]
        paydacoin_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + paydacoin_fee2))
