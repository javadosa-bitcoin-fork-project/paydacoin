...
class WalletTest(PaydacoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 PDC from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 pdc in block rewards plus fees, but
        ...
        # Send 10 PDC with subtract fee from amount
        ...
        # Sendmany 10 PDC with subtract fee from amount
        ...
        fee_rate_sat_vb = 2
        fee_rate_pdc_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_pdc_kvb = Decimal(fee_rate_pdc_kvb) / 1000

        # Test passing fee_rate as a string
        txid = self.nodes[2].sendmany(amounts={address: 10}, fee_rate=str(fee_rate_sat_vb))
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_pdc_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_pdc_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        for target, mode in product([-1, 0], ["pdc/kb", "sat/b"]):
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Paydacoin address
            assert_raises_rpc_error(-5, "Invalid Paydacoin address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Paydacoin address
            assert_raises_rpc_error(-5, "Invalid Paydacoin address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Paydacoin address or script", self.nodes[0].importaddress, "invalid")
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_pdc_kvb))
