#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Paydacoin Core developers
...
TMPDIR_PREFIX = "paydacoin_func_test_"
...
class PaydacoinTestMetaClass(type):
    """Metaclass for PaydacoinTestFramework.

    Ensures that any attempt to register a subclass of `PaydacoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'PaydacoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("PaydacoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("PaydacoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class PaydacoinTestFramework(metaclass=PaydacoinTestMetaClass):
    """Base class for a paydacoin test script.

    Individual paydacoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave paydacoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop paydacoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use paydacoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_paydacoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "paydacoind" + config["environment"]["EXEEXT"],
        )
        fname_paydacoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "paydacoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_paydacoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "paydacoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.paydacoind = os.getenv("PAYDACOIND", default=fname_paydacoind)
        self.options.paydacoincli = os.getenv("PAYDACOINCLI", default=fname_paydacoincli)
        self.options.paydacoinutil = os.getenv("PAYDACOINUTIL", default=fname_paydacoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: paydacoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("PaydacoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'paydacoind', self.options.paydacoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'paydacoin-cli', self.options.paydacoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a paydacoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple paydacoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as paydacoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    paydacoind=self.options.paydacoind,
                    paydacoin_cli=self.options.paydacoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in paydacoin.conf
    ...
    def skip_if_no_paydacoind_tracepoints(self):
        """Skip the running test if paydacoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("paydacoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_paydacoind_zmq(self):
        """Skip the running test if paydacoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("paydacoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if paydacoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("paydacoin-wallet has not been compiled")

    def skip_if_no_paydacoin_util(self):
        """Skip the running test if paydacoin-util has not been compiled."""
        if not self.is_paydacoin_util_compiled():
            raise SkipTest("paydacoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if paydacoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("paydacoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether paydacoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether paydacoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_paydacoin_util_compiled(self):
        """Checks whether paydacoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_PAYDACOIN_UTIL")
