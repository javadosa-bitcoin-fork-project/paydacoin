dnl Copyright (c) 2013-2015 The Paydacoin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

AC_DEFUN([PAYDACOIN_FIND_BDB48],[
  ...
  if test "$use_bdb" = "no"; then
    ...
    if test "$bdbpath" = "X"; then
      ...
      PAYDACOIN_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdbpath}],db_cxx)
      ...
      PAYDACOIN_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdb48path}],db_cxx)
      bdbpath="${bdb48path}"
      use_bdb=yes
    fi
  else
